...
router.get('/', async (req, res) => {
  try {
    ...
    const documentsWithFilename = rows.map(row => ({
      ...row,
      filename: `${row.title}.pdf` // Assuming filename is based on title
    }));
...
router.get('/:id/content', async (req, res) => {
    const { id } = req.params;
    try {
        // Since we don't have the original filename, we'll try to construct it from the title
        //  and attempt to read the file.  This assumes the filename is the title with a .pdf extension.
        //  This is a fallback and might not always be accurate.
        const { rows } = await pool.query('SELECT title FROM documents WHERE id = $1', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Documento no encontrado' });
        }
        const title = rows[0].title;
        const filename = `${title}.pdf`; // Assuming PDF extension. Adjust if necessary.
        ...
        res.status(404).json({ error: 'Documento no encontrado or filename incorrect' });
    }
});
